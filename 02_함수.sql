...
-- TRUNC(숫자 | 컬럼명 [, 위치] : 특정 위치 아래를 절삭) : 절삭 
...
SELECT TO_DATE('241008 163230', 'YYMMDD HH24:MI:SS') FROM DUAL;
-- SQL Error [1861] [22008]: ORA-01861: 리터럴이 형식 문자열과 일치하지 않음
--> 패턴을 적용해서 적성된 문자열의 가가 문자가 어떤 날짜 형식인지 인식 시킴
...
'YYYY"년" MM"월" DD"일"') AS "태어난 생년월일" FROM EMPLOYEE;
...
-- 그룹 함수
-- 하나 이상의 행을 그룹으로 묶어 연산한여 총합, 평균 등의
-- 하나의 결과 행으로 반환하는 함수
