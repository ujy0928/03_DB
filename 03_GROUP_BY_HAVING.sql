...
-- EMPLOYEE 테이블에서
-- 부서별 평균 급여가 3백만원 이상인 부서
-- 부서코드, 평균급여 조회
-- 부서코드 오름차순
SELECT DEPT_CODE, ROUND(AVG(SALARY)) 
FROM EMPLOYEE
--WHERE AVG(SALARY) >= 3000000 --> 한 사람의 급여가 3백만원 이상이라는 조건(요구사항 X)
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE 그룹 중 급여 평균이 3백만원 이상인 그룹 
ORDER BY DEPT_CODE;
...
-- CUBE : GROUP BY 절에 작성된 모든 컬럼의 집계를 처리하는 함수
SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
