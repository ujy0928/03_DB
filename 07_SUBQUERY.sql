...
SELECT EMP_ID, EMP_NAME, JOB_NAME, TO_CHAR(TO_DATE(TO_CHAR(SYSDATE, 'YYMMDD'), 'RRMMDD'), 'YYYY') - TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD'), 'YYYY') 나이
,TO_CHAR(DECODE(BONUS, NULL, SALARY * 12, SALARY * (1 + BONUS) * 12), 'L999,999,999')
FROM EMPLOYEE MAIN
JOIN JOB J ON(J.JOB_CODE = MAIN.JOB_CODE)
WHERE TO_CHAR(TO_DATE(TO_CHAR(SYSDATE, 'YYMMDD'), 'RRMMDD'), 'YYYY') - TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD'), 'YYYY')
= (SELECT MIN(TO_CHAR(TO_DATE(TO_CHAR(SYSDATE, 'YYMMDD'), 'RRMMDD'), 'YYYY') - TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD'), 'YYYY')) 나이
	FROM EMPLOYEE SUB
...
-- 중요한 규칙은 USING절에 명시된 컬럼은 중복되지 않으며, 별칭 없이 바로 사용해야함.
